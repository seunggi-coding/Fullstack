				...
				<table class="table" id="table">
					...
					<c:forEach var="inquiry" items="${inquirys.content}">
						<tbody>
							<tr>
								<td>${inquiry.id}</td>
								<td class="inquiry_title"><a href="/inquiry/${inquiry.id}">${inquiry.title}</a></td>
								<td>${inquiry.users.username}</td>
								<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
										value="${inquiry.createDate}" /></td>
								<td>${inquiry.count}</td>
							</tr>
						</tbody>
					</c:forEach>
				...
				<ul class="pagination justify-content-center">
					<c:choose>
						<c:when test="${inquirys.first}">
							<li class="page-item disabled"><a class="page-link"
								href="?page=${inquirys.number-1}">Previous</a></li>
						</c:when>
						<c:otherwise>
					...
					<c:choose>
						<c:when test="${inquirys.last}">
							<li class="page-item disabled"><a class="page-link"
								href="?page=${inquirys.number+1}">Next</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="?page=${inquirys.number+1}">Next</a></li>
